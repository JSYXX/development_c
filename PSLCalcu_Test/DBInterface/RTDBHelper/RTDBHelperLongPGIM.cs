...
namespace DBInterface.RTDBInterface.LongPGIM
{
    /// <summary>
    /// RTDbHelper:PGIMSpecial
    /// 有关实时数据库PGIM的特别连接的方法。
    /// ——针对PGIM没有连接池，采用长连接的方式来提高访问速度。
    /// ——该连接方法，仅用于PGIM数据库。RTDbType固定为CurrentRTDbType.PGIM;
    /// ——PGIMSpecial借口的对外使用方法都一样。仅需要将命名空间从DBInterface.RTDBInterface改变为DBInterface.RTDBInterface.PGIMSpecial
    /// ——但是实际上执行的内容不一样。
    /// ——在PGIMSpecial中，实际的rtdbhelper是静态的，类初始化时即创建对象。
    /// ——使用构造函数RTDbHelper rtdbhelper = new RTDbHelper()时，相当于logon。
    /// ——要注意的问题，在PGIMSpecial中，类一初始化就获得PGIM的一个实例。这个是否影响DBInterface.RTDBInterface中，rtdbhelper用new获得一个实例后的正常工作。
    /// 
    /// 版本：1.0
    /// 
    /// 特别说明：
    ...
    public class RTDbHelper
    {
        ...
        //PGIMSpecial的rtdbhelper与RTDBInterface的rtdbhelper不同在与，PGIMSpecial是静态对象。
