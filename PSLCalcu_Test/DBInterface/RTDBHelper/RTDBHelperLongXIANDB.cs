...
namespace DBInterface.RTDBInterface.LongXIANDB
{
    /// RTDbHelper:LongXIANDB
    /// 有关实时数据库XIANDB的特别连接的方法。
    /// ——针对XIANDB没有连接池，采用长连接的方式来提高访问速度。
    /// ——该连接方法，仅用于XIANDB数据库特定版本。RTDbType固定为CurrentRTDbType.XIANDB;
    /// ——PISpecial接口的对外使用方法都一样。仅需要将命名空间从DBInterface.RTDBInterface改变为DBInterface.RTDBInterface.LongXIANDB
    /// ——但是实际上执行的内容不一样。
    /// ——在LongXIANDB中，实际的rtdbhelper是静态的，类初始化时即创建对象。
    /// ——使用构造函数RTDbHelper rtdbhelper = new RTDbHelper()时，相当于logon。
    /// ——要注意的问题，在LongXIANDB中，类一初始化就获得PI的一个实例。这个是否影响DBInterface.RTDBInterface中，rtdbhelper用new获得一个实例后的正常工作。
    /// 
    /// 版本：1.0
    /// 
    /// 特别说明：
