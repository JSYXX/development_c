...
namespace PSLCalcu.Module.New_Base_Caculate
{
    public class MPVBaseLong
    {
        ...
        private static DataSet _inputData;
        //多输入参数标签顺序：
        public static DataSet inputData
        {
            get
            {
                return _inputData;
        ...
        public static Results Calcu(DataSet inputs, CalcuInfo calcuinfo)
        {
            ...
            {//读取参数
                ...
                List<MPVBaseMessageOutClass> effectiveData = new DatatableToEntity<MPVBaseMessageOutClass>().FillModel(ds.Tables[0]);
                ...
                MPVBaseMessageOutClass res = MPVBaseCaculate.longMPVBase(effectiveData, N1, N2, N3, k, b, stbl, nostbl);

                //计算结果存入数据库
                bool isok = BLL.AlgorithmBLL.insertLongMPVBase(res, tagid, year, month, day, hour);
                if (isok)
                {
                    return new Results(results, _errorFlag, _errorInfo, _warningFlag, _warningInfo, _fatalFlag, _fatalInfo);
                }
