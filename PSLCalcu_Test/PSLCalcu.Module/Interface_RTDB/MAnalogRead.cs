...
namespace PSLCalcu.Module
{
    /// <summary>
    /// 读取一个模拟量值，保存到概化库   
    /// ——本算法主要的用途是，当前端需要显示过程量实时曲线的时候，将实时数据全部从SIS库存到概化库
    /// ——该模块要慎用，一般仅用于更新速率特别慢的点。
    /// 
    /// 
    /// 版本：1.0
    ///    
    ...
    /// </author> 
    /// </summary>
    public class MAnalogRead : BaseModule, IModule
    {
        ...
        private string _moduleName = "MAnalogRead";
        ...
        private string _moduleDesc = "读取模拟量值";
        ...
        private string _algorithms = "MAnalogRead";
        ...
        private string _outputDescs = "ANGValue";
        ...
        private string _outputDescsCN = "模拟量值";
        ...
        public static Results Calcu(List<PValue>[] inputs, CalcuInfo calcuinfo)
        {
            ...
            try
            {
                //该算法只是把实时数据读取进来，供前端显示实时曲线用。

                //0、输入
                List<PValue> input = new List<PValue>();
                //0.1、输入处理：输入长度。当输入为空时，所有的输出项为0.
                if (inputs == null || inputs.Length == 0 || inputs[0] == null)
                    return new Results(results, _errorFlag, _errorInfo, _warningFlag, _warningInfo, _fatalFlag, _fatalInfo); //如果输入为空，则主引擎已经报警，这里仅需给出非正常状态结果。
                else
                    input = inputs[0];
                //0.2、输入处理：截止时刻值。该算法止时刻点不参与计算。删除
                if (input.Count > 1) input.RemoveAt(input.Count - 1);
                //0.3、输入处理：标志位。该算法考虑标志位不为0的情况，先过滤这些点。
                for (i = input.Count - 1; i >= 0; i--)
                {
                    //20181227修改，不删除，而是把值置为0。交由前端处理状态不为0的值。
                    //if (input[i].Status != 0) input.RemoveAt(i);
                    if (input[i].Status != 0) input[i].Value = 0;
                }
                //0.4、输入处理：过滤后结果。
