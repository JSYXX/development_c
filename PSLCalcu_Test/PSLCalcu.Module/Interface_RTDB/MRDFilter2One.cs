...
namespace PSLCalcu.Module
{
    ...
    /// </author> 
    /// </summary>
    public class MRDFilter2One : BaseModule, IModule, IModuleExPara
    {
        ...
        private string _moduleName = "MRDFilter2One";
        ...
        private string _moduleDesc = "实时数据带滤波求均值";
        ...
        private string _algorithms = "RDFilter2One";
        ...
        private string _moduleParaExample = "N;0;1;0;L";// 注意：如果计算模块需要参数，则该属性必须有值，不能为空。检查程序依据此属性是否为空来决定是否对配置项进行正则检查
        ...
        //——D表示去掉最大最小过滤
        //——还可以添加其他过滤方式
        private string _moduleParaDesc = "过滤方式;二次系数;一次系数；平移系数;用上下限进行过滤的标志";// 注意：如果计算模块需要参数，则该属性必须有值，不能为空。检查程序依据此属性是否为空来决定是否对配置项进行正则检查
        ...
        private Regex _moduleParaRegex = new Regex(@"^(N|F){1}(;[+-]?\d+(\.\d+)?){1}(;[+-]?\d+(\.\d+)?){2}(;L){0,1}$");
        ...
        private string _outputDescs = "RDFilterAvg";
        ...
        private string _outputDescsCN = "实时数据滤波均值";
