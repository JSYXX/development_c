...
namespace PSLCalcu.Module
{
    ...
    /// </author> 
    /// </summary>
    public class MRDSegmentMaxMinAvg : BaseModule, IModule, IModuleExPara
    {
        ...
        private string _moduleName = "MRDSegmentMaxMinAvg";
        ...
        private string _moduleDesc = "(阶梯曲线)实时数据先分割再求带滤波的最大最小均值。";
        ...
        private string _algorithms = "MRDSegmentMaxMinAvg";
        ...
        private string _outputDescs = "RDSegMax;RDSegMin;RDSegAvg";
        ...
        private string _outputDescsCN = "实时数据分割最大值;实时数据分割最小值;实时数据分割均值";
        ...
        public static Results Calcu(List<PValue>[] inputs, CalcuInfo calcuinfo)
        {
            ...
            try
            {
                ...
                pvaluesSpan = SpanPValues4rtdbStep(input, calcuinfo.fstarttime, calcuinfo.fendtime, intervalseconds);    //对数据进行分割
                ...
                List<PValue> resultAvg = new List<PValue>();       //进行计算时，先暂时向result中添加，如果计算到最后不出错，才把result的值给results。如果出错还需要直接返回results
                List<PValue> resultMax = new List<PValue>();       //进行计算时，先暂时向result中添加，如果计算到最后不出错，才把result的值给results。如果出错还需要直接返回results
                List<PValue> resultMin = new List<PValue>();       //进行计算时，先暂时向result中添加，如果计算到最后不出错，才把result的值给results。如果出错还需要直接返回results
                ...
                for (int j = 0; j < spanNumber; j++)
                {
                    ...
                    if (pvaluesSpan[j] == null || pvaluesSpan[j].Count == 0)
                    {
                        ...
                        resultAvg.Add(new PValue(0, calcuinfo.fstarttime.AddSeconds(j * intervalseconds), calcuinfo.fstarttime.AddSeconds((j + 1) * intervalseconds), (long)StatusConst.InputIsNull));
                        resultMax.Add(new PValue(0, calcuinfo.fstarttime.AddSeconds(j * intervalseconds), calcuinfo.fstarttime.AddSeconds((j + 1) * intervalseconds), (long)StatusConst.InputIsNull));
                        resultMin.Add(new PValue(0, calcuinfo.fstarttime.AddSeconds(j * intervalseconds), calcuinfo.fstarttime.AddSeconds((j + 1) * intervalseconds), (long)StatusConst.InputIsNull));
                    ...
                    double totalspan = 0;
                    double avg = 0;
                    double max = pvs[0].Value;
                    double min = pvs[0].Value;
                    
                    DateTime maxdate = pvs[0].Timestamp;
                    DateTime mindate = pvs[0].Timestamp;


                    for (i = 0; i < pvs.Count; i++)
                    {
                        sum = sum + pvs[i].Value*pvs[i].Timespan;
                        totalspan = totalspan + pvs[i].Timespan;
                        if (pvs[i].Value > max)
                        {
                            max = pvs[i].Value;
                            maxdate = pvs[i].Timestamp;
                        }
                        if (pvs[i].Value < min)
                        {
                            min = pvs[i].Value;
                            mindate = pvs[i].Timestamp;
                        }
                    }
                    avg = sum / totalspan;                       //对剩下的值求平均
                    double filteravg = A * avg * avg + B * avg + C;   //对平均值进行变换。注意一般情况是A=0；B=1；B=0；D=0，即filtervalue=ave

                    //如果参数中含有第四个变量，且变量值为L，则对计算结果用上下限进行过滤
                    if (FilterWithLimit == "L")
                    {
                        if (filteravg < calcuinfo.sourcetagmrb)
                            resultAvg.Add(new PValue(filteravg, calcuinfo.fstarttime.AddSeconds(j * intervalseconds), calcuinfo.fstarttime.AddSeconds((j + 1) * intervalseconds), (long)StatusConst.InputOverLimit));
                        else if (filteravg > calcuinfo.sourcetagmre)
                            resultAvg.Add(new PValue(filteravg, calcuinfo.fstarttime.AddSeconds(j * intervalseconds), calcuinfo.fstarttime.AddSeconds((j + 1) * intervalseconds), (long)StatusConst.InputOverLimit));
                        else
                            resultAvg.Add(new PValue(filteravg, calcuinfo.fstarttime.AddSeconds(j * intervalseconds), calcuinfo.fstarttime.AddSeconds((j + 1) * intervalseconds), 0));
                    }
                    else//没有参数L，不过滤
                    {
                        resultAvg.Add(new PValue(filteravg, calcuinfo.fstarttime.AddSeconds(j * intervalseconds), calcuinfo.fstarttime.AddSeconds((j + 1) * intervalseconds), 0));                       
                    }
                    resultMax.Add(new PValue(max, maxdate, calcuinfo.fstarttime.AddSeconds((j + 1) * intervalseconds), 0));
                    resultMin.Add(new PValue(min, mindate, calcuinfo.fstarttime.AddSeconds((j + 1) * intervalseconds), 0));
        ...
        //*****************特别注意，这里的分割程序要与并发计算引擎中，实时数据的分割计算程序一致。如果并发计算引擎计算方法进行更新，这里要一起更新
