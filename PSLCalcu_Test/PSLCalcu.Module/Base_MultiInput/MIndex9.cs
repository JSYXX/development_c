...
namespace PSLCalcu.Module
{
    /// <summary>
    /// 排序(9个以内的输入变量)
    /// ——对输入的一组标签(不大于10个)的值进行排序
    /// ——本算法支持排序的输入标签数量不能大于10个。
    /// ——本算法仅有一个9位数的整形输出，每一位分别代表对应标签在当期的序位。序位是1~9.
    /// ——如果实际参与排序的标签数量不足9个，则相应的有效输出也不足9。应该使用algorithmsflag参数来控制有效输出的保存。
    /// ——不能支持10个数字，如果是十个数字，序位是0~9，当第一个位置出现0时，无法反应到计算结果的整形数字中。
    /// 版本：1.1
    ///    
    /// 修改纪录   
    /// 
    ...
    /// </author> 
    /// </summary>
    public class MIndex9:BaseModule, IModule, IModuleExPara
    { 
        ...
        private string _moduleName = "Index9";
        ...
        private string _moduleDesc = "排序(9个以内的输入变量)";
        ...
        private string _algorithms = "Index9";
        ...
        private string _outputDescs =   "Index9";
        ...
        private string _outputDescsCN = "标签排序结果";
        ...
        public static Results Calcu(List<PValue>[] inputs, CalcuInfo calcuinfo)
        {
            ...
            try 
            {
                ...
                for(i=0;i<inputs.Length;i++)
                {
                    result=result*10+tagindex[i];
                }

                results = new List<PValue>[1];
                results[0] = new List<PValue>();
                results[0].Add(new PValue(result, calcuinfo.fstarttime, calcuinfo.fendtime, 0));
