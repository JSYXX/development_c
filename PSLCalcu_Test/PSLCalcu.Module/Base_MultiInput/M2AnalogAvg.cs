...
{
    /// <summary>
    /// 两个模拟量值序列求平均，到一个新的模拟量值序列   
    /// ——两个输入模拟量，第一个模拟量和第二个模拟量的实时平均值，得到的结果作为新的模拟量返回。
    /// ——输出是两个模拟量的均值。
    /// 
    /// 版本：1.0
    ///    
    ...
    /// </author> 
    /// </summary>
    public class M2AnalogAvg : BaseModule, IModule, IModuleExPara
    {
        ...
        private string _moduleName = "M2AnalogAvg";
        ...
        private string _moduleDesc = "两个模拟量的平均值，得到一个新的模拟量";
        ...
        private string _algorithms = "2AnalogAvg";
        ...
        private string _outputDescs = "2ANGAvg";
        ...
        private string _outputDescsCN = "模拟量平均值";
            ...
            {              
                ...
                for (i = 0; i < spanconcat.Count; i++)
                {
                    ...
                    {//如果当前时间不等于上一次时间，说明上一个时刻的所有状态变化处理完毕，flagSeries以代表上一个时刻最后状态
                        //2.1 写入上次结果
                        //currentAvg = (valueSeries[0] + valueSeries[1])/2;
                        currentAvg = k * ((valueSeries[0] + valueSeries[1]) / 2) + b;
                        //填写上上时刻点的结束时间
