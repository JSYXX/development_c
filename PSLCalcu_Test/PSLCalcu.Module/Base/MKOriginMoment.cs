...
namespace PSLCalcu.Module
{
    ...
    /// </author> 
    /// </summary>
    public class MKOriginMoment : BaseModule, IModule, IModuleExPara
    {
        #region 计算模块信息：模块名称k阶原点距、输出项个数1、输出项名称kOriginMoment、输出项写入的数据表名称plsdata
        //这些信息，每个计算模块各不相同，用于区别不同的算法实现类。
        //这些信息，仅供计算引擎获取计算模块信息时使用，可以通过反射的方式，创建实例来调用，并获取其值。
        //在真正计算时，不需要使用这些方法，因此这些方法无需静态。
        private string _moduleName = "kOriginMoment";
        ...
        private string _moduleDesc = "k阶原点距 ";
        ...
        private string _algorithms = "kOriginMoment";
        ...
        private string _outputDescs = "kOriginMoment";
        ...
        private string _outputDescsCN = "k阶原点距";
        ...
        public static Results Calcu(List<PValue>[] inputs, CalcuInfo calcuinfo)
        {
            ...
            {                
                ...
                #region 初始化
                PValue kOriginMoment = new PValue(0, calcuinfo.fstarttime, calcuinfo.fendtime, 0);
                List<PValue>[] results = new List<PValue>[1];
                int len = input.Count;
               
                //Console.WriteLine("请输入计算k阶原点距时所需参数k：\n");
                ...
                {
                    //k阶原点矩
                    kOriginMoment.Value += System.Math.Pow(input[i].Value, k);
                }
                //k阶原点矩
                kOriginMoment.Value = kOriginMoment.Value / len;


                results[0] = new List<PValue>();
                results[0].Add(kOriginMoment);
