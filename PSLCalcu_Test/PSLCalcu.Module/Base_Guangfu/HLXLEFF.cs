...
{
    public class HLXLEFF : BaseModule, IModule
    {
        ...
        //y=k(x1+x2)+b;
        private string _moduleName = "HLXLEFF";
        ...
        private string _moduleDesc = "汇流箱损失";
        ...
        private string _algorithms = "HLXLEFF";
        ...
        private string _outputDescs = "HLXLEFF";
        ...
        private string _outputDescsCN = "汇流箱损失";
        ...
        public static Results Calcu(List<PValue>[] inputs, CalcuInfo calcuinfo)
        {
            ...
            try
            {
                ...
                PValue HLXLEFF = new PValue(0, calcuinfo.fstarttime, calcuinfo.fendtime, 0);
                results = new List<PValue>[1];
                //(Z - X) / Z *100
                //开始计算
                //第一个输入为发电量  第二个输入为辐照量
                //辐照量每隔15分钟取一个值
                double x = inputs[0][0].Value;
                double z =0;
                double result;
                for (int j = 1; j < inputs.Length; j++)
                {
                    z = z + inputs[j][0].Value;
                }
                result = (z - x) / z * 100;
                HLXLEFF.Value = result;
                results[0] = new List<PValue>();
                results[0].Add(HLXLEFF);
